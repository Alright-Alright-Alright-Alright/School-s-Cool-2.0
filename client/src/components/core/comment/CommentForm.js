...
function CommentForm({ postId }) {
  ...
  return (
    <form className="flex px-5 pb-5 pt-3" onSubmit={handleFormSubmit}>
      <img
        className="w-10 h-10 rounded-full mr-2 border-grey-light border-2"
        src={`${user?.imageUrl}`}
        alt="profile"
      />
      <input
        className=" border-grey-light border-2 bg-white w-full rounded-full text-sm pl-3 placeholder-grey-dark"
        type="text"
